...
class RPPORLPlayer(Gen8EnvSinglePlayer):
    ...
    def embed_battle(self, battle):
        ...
        state= np.concatenate([
            [NAME_TO_ID_DICT_DET[str(battle.active_pokemon).split(' ')[0]]],
            [NAME_TO_ID_DICT_DET[str(battle.opponent_active_pokemon).split(' ')[0]]],
...
if __name__ == "__main__":
    ...
    if args.train:
        env_player = RPPORLPlayer(battle_format="gen8ou", team=OUR_TEAM_DET, mode = "train")
    else: env_player = RPPORLPlayer(battle_format="gen8ou", team=OUR_TEAM_DET, mode = "val")

    second_opponent = RandomPlayer(battle_format="gen8ou", team=OP_TEAM_DET)
    opponent= MaxDamagePlayer(battle_format="gen8ou", team=OP_TEAM_DET)


    if args.saved: 
        modelfolder = args.model_folder
    else: 
        model = RPPO("MlpLstmPolicy", env_player, gamma=args.gamma, verbose=0)

# Train
    def RPPOtraining(player):
